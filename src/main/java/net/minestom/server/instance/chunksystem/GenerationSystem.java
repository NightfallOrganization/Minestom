...
final class GenerationSystem {
    ...
    public @NotNull GeneratorTask publishUpdate(@NotNull ChunkManagerImpl.Entry entry, @NotNull ChunkClaimImpl claim) {
        ...
        var node = queue.add(claim.priority(), request);
        var priority = claim.priority();
        return new GeneratorTaskImpl(queue, priority, node);
    }

    public void publishRadius(ChunkManagerImpl.Entry entry, ChunkClaimImpl claim) {
        var request = new PropagationRequest(entry, PropagationRequest.Direction.ALL);
    ...
    private record PropagationRequest(ChunkManagerImpl.Entry entry, Direction direction) implements Request {
        public enum Direction {
            ALL, POSITIVE_X, POSITIVE_Z, NEGATIVE_X, NEGATIVE_Z
        }
    }

    private record RemovePropagationRequest(ChunkManagerImpl.Entry entry, PropagationRequest.Direction direction) implements Request {
